...
public class CasetaService {
	...
	public Optional<Caseta> findCasetaById(int id) {
		Optional<Caseta> casetaOpt = casetaRepository.findById(id);

		if (casetaOpt.isPresent()) {
			Caseta caseta = casetaOpt.get();

			// Forzar la carga de la colección LAZY
			Hibernate.initialize(caseta.getSolicitudesLicencia());
		}
	...
	/**
	 * Añade un Socio a una Caseta existente (Relación Unidireccional Caseta -->
	 * Socio).
	 */
	@Transactional
	public Caseta addSocio(int socioId) {
		...
		// Obtenemos el tamaño actual de la lista de socios, le sumamos uno ya que
		// despúes queremos añadir un socio
		...
		if (sociosActuales >= aforoMaximo) {
			throw new IllegalStateException(
					"Error de negocio: La caseta  ha alcanzado su aforo máximo (" + aforoMaximo + " socios).");
		}
		...
		if (!caseta.getSocios().contains(socio)) {
			...
			throw new IllegalStateException(
					"Error de negocio: El socio con ID " + socioId + " ya es miembro de la caseta.");
		}
	...
	/**
	 * Elimina un Socio a una Caseta existente (Relación Unidireccional Caseta -->
	 * Socio).
	 */
	@Transactional
	...
	/**
	 * Añade un Producto a una Caseta existente (Relación Unidireccional Caseta ->
	 * Producto).
	 */
